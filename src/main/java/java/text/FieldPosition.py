...
# ORIGINAL LINE: public FieldPosition(int field)
    ...
    #     
# JAVA TO PYTHON CONVERTER TASK: There is no Python equivalent to multiple constructors:
# ORIGINAL LINE: public FieldPosition(Format.Field attribute)
    ...
    #     
# JAVA TO PYTHON CONVERTER TASK: There is no Python equivalent to multiple constructors:
# ORIGINAL LINE: public FieldPosition(Format.Field attribute, int fieldID)
    def __init__(self, attribute, fieldID):
        self._initialize_instance_fields()
    ...
    #     
    def equals(self, obj):
        temp_var = isinstance(obj, FieldPosition)
        other = obj if temp_var else None
        if not(temp_var):
            return False
        if not java.util.Objects.equals(self._attribute, other._attribute):
            return False
        return (self.beginIndex == other.beginIndex and self.endIndex == other.endIndex and self.field == other.field)

    #    *
    #     * {@return a hash code for this FieldPosition}
    #     
    def hashCode(self):
        return (self.field << 24) | (self.beginIndex << 16) | self.endIndex
    ...
    #     
    def toString(self):
        return getClass().getName() + "[field=" + str(self.field) + ",attribute=" + self._attribute + ",beginIndex=" + str(self.beginIndex) + ",endIndex=" + str(self.endIndex) + ']'
...
# JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def _matchesField(self, attribute):
        if self._attribute is not None:
            return self._attribute is attribute
        return False
    ...
    #     
# JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
    def _matchesField(self, attribute, field):
        if self._attribute is not None:
            return self._attribute is attribute
    ...
    #     * the FieldPosition.
    #     
    class Delegate(Format.FieldDelegate):

        def __init__(self, outerInstance):
            # instance fields found by Java to Python Converter:
            self._encounteredField = False
...
# JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
        def formatted(self, attr, value, start, end, buffer):
            if not self._encounteredField and outerInstance._matchesField(attr):
                outerInstance.setBeginIndex(start)
                outerInstance.setEndIndex(end)
                self._encounteredField = (start != end)

# JAVA TO PYTHON CONVERTER TASK: Python does not allow method overloads:
        def formatted(self, fieldID, attr, value, start, end, buffer):
            if not self._encounteredField and outerInstance._matchesField(attr, fieldID):
                outerInstance.setBeginIndex(start)
                outerInstance.setEndIndex(end)
                self._encounteredField = (start != end)
